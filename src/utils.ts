...
export function parseAvailableTests(doc = document) {
  ...
  return main
    ...
    .map((test) => {
      const title = test
        .find((el) => el.nodeName === "H3")
        ?.textContent.replace("Test: ", "");
      const id = test.find(
        (el) => el.nodeName === "DIV" && el.classList.contains("collapse"),
      )?.id;
      const link = test
        .find((el) => el.nodeName === "H3")
        ?.querySelector("a")?.href;
      const openFrom = strToDate(
        test
          .find((el) => el.nodeName === "TABLE")
          ?.querySelector("tr:first-child > td:nth-child(2)").innerText,
      );
      const openTo = strToDate(
        test
          .find((el) => el.nodeName === "TABLE")
          ?.querySelector("tr:nth-child(2) > td:nth-child(2)").innerText,
      );
...
export function convertDate<T extends Date | string>(
  date: T,
): T extends Date ? string : Date {
  if (typeof date === "string") {
    return new Date(date) as T extends Date ? string : Date;
  } else if (date instanceof Date) {
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}` as T extends Date
      ? string
      : Date;
  } else {
    return null;
